...
const AssetManagement = ({ darkMode }) => {
  ...
  const handleDeleteAsset = async (assetId) => {
    try {
      const response = awaitfetch(`http://se160590.fg.rbc.com:5000/api/assets/${assetId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const handleStageUpdate = async (assetId, stage, status) => {
    try {
      const response = awaitfetch(`http://se160590.fg.rbc.com:5000/api/assets/${assetId}/stage`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleFetchAssetInfo = async (assetId, assetName, loginId, businessGroup, ynx1cDate, imagingDate, status) => {
    ...
    //IMAGING
    try {
      const response = awaitfetch('http://se160590.fg.rbc.com:5000/api/run-powershell', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (imagingDate){
        const updateResponse = awaitfetch(`http://se160590.fg.rbc.com:5000/api/assets/${assetId}/imagingdate`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  try {
    ...
    if (ynx1cDate){
      const updateResponse = awaitfetch(`http://se160590.fg.rbc.com:5000/api/assets/${assetId}/ynx1cdate`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
...
/*
      ...
      console.log('Intializing RSA Check...')
      ...
      if (response.ok && sid) {
        ...
        const rsaResponse = await fetch('http://se160590.fg.rbc.com:5000/api/run-rsacheck', {
            ...
            body: JSON.stringify({
                script: `Invoke-Command -ComputerName ${assetName} -ScriptBlock { Get-ItemProperty -Path 'REGISTRY::HKEY_USERS\\${sid}\\SOFTWARE\\RSA\\Software Token\\Library' | Select-Object -ExpandProperty DefaultToken }`
            }),
        });
...
*/
      ...
      try {
        ...
        status = "TRUE";
        if (bundlesCheck){
          const updateResponse = awaitfetch(`http://se160590.fg.rbc.com:5000/api/assets/${assetId}/tsbundle`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
        ...
        try {
          ...
          const checkBusinessApps = (businessGroup, installedApps) =>{
            let requiredApps = [];
            if(businessGroup.includes('BCS - Operations')) {
              requiredApps = ['ADC3', 'ADC4']
            }
            else if (businessGroup.includes('GM - Research')){
              requiredApps = ['XXU4', 'TB81', 'FEJ1', 'XGW1', 'TB82', 'VXN2', 'SEW0']
            }
            else if (businessGroup.includes('GM - S&T')){
              requiredApps = ['MDM1', 'ZPGS', 'UVDC', 'XXU4', 'TB81', 'FEJ1']
            }
            else if (businessGroup.includes('GM - Origination')){
              requiredApps = ['MDM1', 'ZPGS', 'UVDC', 'ZNK1', 'TB81', 'TB82']
            }
            else if (businessGroup.includes('GM - CFG')){
              requiredApps = ['MDM1', 'FEJ1', 'ZPGS', 'ZPG1', 'ZPGA', 'ZPGR', 'ZMW0', 'U951', 'PTY1', 'MH8F', 'WPq14', 'MH052', 'ZNK1', 'XTC1']
            }
            else if (businessGroup.includes('GM - Quant Trading Program')){
              requiredApps = ['MDM1', 'FEJ1', 'ZPGS', 'ZPG1', 'ZPGA', 'ZPGR', 'ZMW0', 'U951', 'PTY1', 'MH8F', 'WPq14', 'MH052', 'ZNK1', 'XTC1']
            }
            else if (businessGroup.includes('Cash Management')){
              requiredApps = ['FEJ1', 'TF51']
            }
            else if (businessGroup.includes('CB')){
              requiredApps = ['XXU4', 'UVDC', 'YWP2', 'YWP0', 'YRC0', 'DE04', 'SDQ3']
            }
            else if (businessGroup.includes('GIB')){
              requiredApps = ['XXU4', 'UVDC', 'TB81', 'FEJ1', 'TB82', 'VMI5', 'SEW0']
            }
            else if (businessGroup.includes('GRM')){
              requiredApps = ['FEJ1', 'YWP2', 'YWP0', 'UVDC']
            }
            else if (businessGroup.includes('Internal Audit')){
              requiredApps = ['FEJ1', 'TF51']
            }
            else if (businessGroup.includes('MF')){
              requiredApps = ['VZE4', 'MI01P', 'XXU4', 'UVDC', 'MDM1', 'ZJU2', 'VZE6', 'TB81', 'TB82' ]
            }
            else if (businessGroup.includes('MF - Community Investments')){
              requiredApps = ['VZE4', 'MI01P', 'XXU4', 'UVDC', 'MDM1', 'ZJU2', 'VZE6', 'TB81', 'TB82' ]
            }
            else if (businessGroup.includes('RECP')){
              requiredApps = ['FEJ1', 'TF51']
            }
          ...
          if (bundlesCheck){
            ...
            const updateResponse = awaitfetch(`http://se160590.fg.rbc.com:5000/api/assets/${assetId}/bscbundle`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
              },
  ...
  const updateAssetDetails = async (employeeId, userInfoOutput) => {
    ...
    if (assetToUpdate) {
        try {
            ...
            const response = awaitfetch(`http://se160590.fg.rbc.com:5000/api/assets/${assetToUpdate.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
      ...
      <h1 className="mt-10 text-3xl font-bold mb-4 text-center text-gray-900 dark:text-gray-100">Asset Readiness</h1>
       ...
       <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-gray-100 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          {/* Business Bundle */}
          <div className="bg-gray-100 dark:bg-gray-700 shadow p-4 rounded-lg text-center">
            <h3 className="text-md font-semibold mb-1 text-gray-900 dark:text-gray-100">Business Bundles</h3>
                ...
                <div>
                  {editAssetId === asset.id ? (
                    ...
                    <>
                      ...
                      <p className={`text-sm text-gray-900 dark:text-gray-100`}>Batch Date: {newDate(asset.batch_date).toLocaleDateString()}</p>
